...
describe('areArraysEqual', () => {
  ...
  describe('node comparator', () => {
    test('the same nodes', () => {
      expect(areArraysEqual([text, text], [text, text], areNodesEqual)).toBe(
        true,
      )
    })
    test('identical nodes', () => {
      expect(
        areArraysEqual([text, text], [text, identicalText], areNodesEqual),
      ).toBe(true)
    })
    test('different nodes', () => {
      expect(
        areArraysEqual([text, text], [text, differentText], areNodesEqual),
      ).toBe(false)
    })
  })
})
  ...
  (_: string, customGetAttributeNames: any) => {
    ...
    describe('deep', () => {
      ...
      test('extraneous child', () => {
        ...
        ;((differentRootNode.lastChild as Node).lastChild as Node).appendChild(
