...
export function ApprovalVoteChart({ proposal }: ResultProps) {
  const { processedData, choices } = useMemo(() => {
    ...
    proposal.votes.forEach((vote) => {
      ...
      // Ensure choice is an array of approved choices and convert to string
      const approvedChoices = Array.isArray(vote.choice)
        ? vote.choice.map(String)
        : [String(vote.choice)];

      approvedChoices.forEach((choiceIndexString) => {
        const choiceIndex = parseInt(choiceIndexString) - 1;
        if (choiceVotes[choiceIndex] !== undefined) {
          const previousTotal =
          ...
          choiceVotes[choiceIndex].push({
            timestamp,
            totalVotingPower: previousTotal + totalVotingPower, // Add full voting power
          });
        } else {
          console.warn(
