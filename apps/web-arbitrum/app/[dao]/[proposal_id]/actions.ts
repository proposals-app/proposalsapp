...
export async function getProposalAndGroup(
  ...
  return otel("get-proposal-and-group", async () => {
    console.time("getProposalAndGroup");
    // Get the DAO first
    const dao = await db
      .selectFrom("dao")
      .where("slug", "=", slug)
    ...
    console.timeEnd("getProposalAndGroup");
    return {
      dao,
      group: matchingGroup,
    };
    ...
    const topics =
      topicIds.length > 0
        ? await db
            ...
            .execute()
        : [];
    console.timeEnd("getGroupDetails");
    return {
      ...group,
      proposals,
      topics,
