...
use super::super::super::typings::rindexer::events::arbitrum_core_governor::{no_extensions, ArbitrumCoreGovernorEventType, ProposalCanceledEvent, ProposalCreatedEvent, ProposalExecutedEvent, ProposalExtendedEvent, ProposalQueuedEvent, VoteCastEvent, VoteCastWithParamsEvent};
use crate::{
    extensions::{
...
fn get_votes_dao_indexer_id() -> ActiveValue<Uuid> {
    DAO_INDEXER_ID_MAP
        ...
        .lock()
        .unwrap()
        .get(&IndexerVariant::ArbCoreArbitrumVotes)
        .map(|id| Set(*id))
        .unwrap_or(NotSet)
}
    ...
    ArbitrumCoreGovernorEventType::VoteCast(
        VoteCastEvent::handler(
            |results, context| async move {
                ...
                rindexer_info!("ArbitrumCoreGovernor::VoteCast - {}", "INDEXED".green(),);

                match store_votes(votes).await {
                    Ok(_) => rindexer_info!("ArbitrumCoreGovernor::VoteCast - {}", "STORED".blue(),),
                    Err(_) => rindexer_info!("ArbitrumCoreGovernor::VoteCast - {}", "NOT STORED".red(),),
                }
