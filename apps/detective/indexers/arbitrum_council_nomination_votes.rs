...
use arbitrum_security_council_nominee_election::VoteCast;
...
use std::sync::Arc;
use tracing::info;
use crate::indexers::arbitrum_council_nomination_proposals::arbitrum_security_council_nominee_election::Date;

sol!(
    #[allow(missing_docs)]
    #[sol(rpc)]
    arbitrum_security_council_nominee_election,
    "./abis/arbitrum_security_council_nominee_election.json"
);

pub struct ArbitrumCouncilNominationVotesIndexer;

impl ArbitrumCouncilNominationVotesIndexer {
...
#[async_trait]
impl Indexer for ArbitrumCouncilNominationVotesIndexer {
...
#[async_trait]
impl VotesIndexer for ArbitrumCouncilNominationVotesIndexer {
    async fn process_votes(
        &self,
        indexer: &dao_indexer::Model,
        _dao: &dao::Model,
    ) -> Result<ProcessResult> {
        info!("Processing Arbitrum Council Nomination Votes");
        ...
        let contract = arbitrum_security_council_nominee_election::new(address, arb_rpc.clone());
...
//         match ArbitrumCouncilNominationVotesIndexer.process_votes(&indexer, &dao).await {
