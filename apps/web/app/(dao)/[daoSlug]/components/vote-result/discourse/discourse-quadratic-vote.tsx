...
export const DiscourseQuadraticVote = ({
  result,
  currentTime,
  width = 1200,
}: QuadraticVoteProps) => {
  ...
  return (
    <div tw='bg-[rgba(239,239,239,0.25)] w-full border border-[#e9e9e9] flex'>
      ...
      >
        {/* First row when narrow: Status and Vote button */}
        <div
          tw='flex items-center justify-between'
          style={{
            width: '100%',
            marginBottom: isNarrow ? '0' : '0',
          }}
        >
          ...
          {/* Vote button - shows in first row when narrow */}
          {isNarrow && (
        ...
        {/* Center section - Vote results (second row when narrow) */}
        {!isNarrow && (
          <div
            tw='flex flex-col'
            style={{
              ...
              {Object.entries(voteSegments).map(([choiceKey, segments]) => {
                ...
                const widthPercentage =
                  totalVotingPower > 0 && choiceTotal >= 0
                    ...
                    : 0;
                if (widthPercentage === 0 || isNaN(widthPercentage))
                  return null;
                return (
                  <div
                    key={choiceKey}
        ...
        {/* Vote results when narrow (second row) */}
        {isNarrow && (
          <div tw='flex flex-col w-full' style={{ gap: '4px' }}>
            {/* Progress bar */}
            <div
              tw='flex w-full bg-[#EEE] overflow-hidden'
              style={{ height: '12px' }}
